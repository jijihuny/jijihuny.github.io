...
4. 이 때 데이터베이스는 안정적인 트랜잭션을 위해 다음과 같은 기능을 제공해야 한다.
    ...
    ---
    **Redo**
    DBMS로부터 **commit이 된 변경사항**이 반드시 **영구적(durability)으로 반영**되도록 보장하는 과정

    ---
    **Undo**
    **부분적으로 완료된 트랜잭션이 중단(abort)**되면 부분완료된 결과가 데이터베이스의 **상태를 변경하지 않도록 보장**하는 과정

5. 데이터베이스에서 트랜잭션을 복구해야하는 상황은 크게 2가지로
    - 트랜잭션 과정 일부에 문제(무결성 제약 위반, 데드락 등)가 생겨 트랜잭션을 더이상 진행할 수 없어 atomicity와 consistency를 보장할 수 없는 경우
    - 어떤 이유로 DBMS가 의도치 않게 종료되어 데이터베이스의 상태가 더 이상 트랜잭션의 atomicity, consistency, durability를 보장할 수 없는 경우

6. 변경된 레코드(dirty record)를 디스크에 쓰는 시점에 따라 메모리 버퍼 관리 정책이 나뉘게 되는데 이는 다음과 같다.
...
7. RDBMS의 경우 대부분 STEAL + NO-FORCE 정책을 채택하여 추가적인 복구 알고리즘을 사용하는데, 
...
10. 약간의 추가적 논의로 lock과 latch를 알아보자. lock과 latch는 모두 synchronization mechanism의 일종으로 공유 자원에 대한 접근을 통제, 관리하는 용도이다. lock은 아마 다들 많이 들어봤을텐데 latch는 무엇일까?
    - latch
...
ARIES의 핵심 아이디어를 5가지로 요약하면 다음과 같다.
...
트랜잭션이 작업을 수행하다 완료 혹은 롤백되는 경우(normal operation)를 살펴보자.

## Page Updates

먼저 페이지 변경 작업의 과정을 간단히 생각해보자. 

```python
def update(trans_id, page_id, data):
    page = fix_and_latch(page_id, 'X')
    # 페이지가 버퍼에 캐시되어있지 않을 경우 적재.
    # 버퍼 매니저는 필요할 경우 dirty page table에 recLSN(end of log)를 기록한다.
    # 페이지 데이터 변경 작업을 위해 eXclusive latch를 건다.
    write(page, data)
    log_lsn = log_write('update', page_id, active_trans_table[trans_id].last_lsn, page_id, data)
    # 로그 작성(WAL)
    # 페이지 변경 정보의 일관성을 위해 로그 작성은 
    # latch를 유지한 상태로 이루어진다.
...
- partial rollback이란 것은 하나의 트랜잭션 안에 체크포인트를 저장하여, 부분 작업에 문제가 생길 경우 해당 체크포인트까지 partially rollback을 진행하고 다시 forward processing을 진행하는 것이다. 

- 앞서 본 pseudo code를 보면 save_lsn을 통해 partial rollback을 위한 체크포인트를 명시해놨음을 알 수 있다. 이러한 체크포인트는 사용자 레벨에서 특정 지점에 설정된다.
...
- CLR을 정의하는 이유는 
    ...
    2. Partial rollback 이후 혹은 rollback 진행 중 발생한 시스템 중단 이후 재시작 시점에 회복작업을 수행하며 반복적인 undo를 방지하여 오버헤드를 줄이고, Idempotency를 보장하기 위함이다.
    3. 또한 이후 얘기할 Redo Pass에서 CLR는 redo-only log로서 취급되는데,
        - 이를 통해 undo 작업에서 디스크 출력 작업을 lazy하게 수행가능하기 때문에 batch 작업 등을 통해 I/O 작업량을 줄일 수 있고,
        - Crash 시점에 데이터베이스의 상태를 완전히 복구하여 Undo Pass에서 트랜잭션의 세부적인 진행 상태를 신경쓰지 않도록 간단하게 만들어준다.
...
따라서 DBMS는 Checkpoint 로그 이전에 작성된 로그들이 이후 복구 작업에 필요하지 않은 상태를 보장해야 하고, 이를 위해 Checkpointing을 하는 동안 데이터베이스의 잠재적인 상태변화를 방지한다.

따라서 checkpoint의 직관적인 설계는
- 새로운 트랜잭션이 시작할 수 없도록 방지하고,
- 현재 진행 중이던 트랜잭션이 모두 안정적으로 완료될 수 있도록 보장하고,
- Dirty page가 모두 디스크로 flush될 수 있도록 기다려야 한다.

결국 Active transaction들의 완료를 기다리고, dirty page들이 모두 flush될 때까지 새로운 작업이 모두 제한되어 performance의 하락이 나타날 수 밖에 없다.

이를 위해 등장한 것이 Fuzzy Checkpoint 개념이다.

Fuzzy Checkpoint의 핵심적인 고민은 결국 checkpoint를 수행하는 동안 어떻게하면 1. active transaction을 허용하면서 2. checkpoint log 작성 시점에 안정적인 데이터베이스 snapshot을 보장할 수 있는가?이다.

Fuzzy Checkpoint의 아이디어는 듣고 보면 간단한데,
checkpoint를 begin과 end로 분리하는 것이다.
...
- Pseudo code를 살펴보다보면, end_chkpt 시점에 기록한 정보(rec_lsn 등)를 계속 변경할 수 있는 걸 알 수 있는데, 이는 end_chkpt 로그를 위한 정보를 construction한 시점부터 써지는 모든 로그가 end_chkpt에 반영되지 않기 때문이다. 이 때 end_chkpt를 stable storage에 쓰는 동안 발생하는 시간차가 문제가 아닐지 걱정되지만 문제가 없는 걸 알 수 있다.
...
1. commit되었으나 dirty page가 디스크에 반영되지 않은 작업이 durable하게 적용되도록 보장함은 물론, 
2. uncommitted 변경사항을 모두 반영하여 중단 시점의 상황을 완전히 재현한다.
...
- Redo Phase에서 병렬처리를 할 수 있는 부분이 있을까? Redo 작업 자체는 로그 순서대로 진행해야만 한다. 정답은 dirty pages이다. dirty_page_table의 entry가 되는 페이지는 결국 디스크로부터 읽어와야만 한다. 로그에 필요할 때마다 순서대로 불러오는 것보다 asynchronous I/O 작업을 통해 불러오는 것이 전체 페이지를 더 빨리 로딩할 수 있고, redo 작업에 아무런 문제가 없음을 알 수 있다.
...
- Undo Phase에서 병렬처리를 할 수도 있을까? 각 트랜잭션의 Undoing 자체는 하나의 프로세스로 수행되어야 함을 알 수 있다. 정답은 Redo Phase와 비슷하다. 대신 이번엔 dirty page table이 아니라 로그 체인(Linked List)를 이용해야 한다. 목표는 필요한 페이지를 ansynchronous I/O 작업으로 미리 불러오는 것이다. 따라서 버퍼 페이지에 대한 undoing은 잠깐 미뤄두고, 먼저 필요한 CLR을 작성하는 처리를 한 다음, 필요한 페이지 목록을 미리 불러오는 것이 더 효율적일 것이다.