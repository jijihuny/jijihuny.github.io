...
w&: V \rightarrow \R = cost \space of \space edge\\
f_i^0 &= w(1,i)\\
...
$f_v^{k}=$최대 k개의 간선을 고려한 최단거리경로임을 보이면 된다.
...
$\forall v \in V,$ $f_v^{k-1}$가 최대 k-1개의 간선을 이용한 최단거리경로라고 가정하자.
...
$v_0=v_k$가 자명하기 때문에 (3)의 양변 첫항을 제거할 수 있다.
